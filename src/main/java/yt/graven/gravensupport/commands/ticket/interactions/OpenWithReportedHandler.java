...
public class OpenWithReportedHandler implements InteractionAction<ButtonInteractionEvent> {
  ...
  public void run(ButtonInteractionEvent event) throws TicketException, IOException {
    ...
    if (manager.exists(user)) {
      embeds.errorMessage("Un ticket avec cet utilisateur existe déjà !").reply(event).queue();
    ...
    ticket.forceOpening(
        event.getUser(), new TicketOpeningReason.Simple("Discussion suite à un signalement"));
    ...
    MessageFactory.create()
        .addEmbeds(
            new EmbedBuilder()
                ...
                .setDescription(
                    "Ce ticket a été ouvert suite à un signalement de %s (%s)"
                        .formatted(
                            event.getUser().getAsMention(), event.getChannel().getAsMention())))
        .send(ticket.getTo())
        .complete()
        .pin()
        .queue();
    ...
    event
        ...
        .editMessageComponents(
            ActionRow.of(
                Button.of(
                    ButtonStyle.LINK,
                    ticket.getTo().getJumpUrl(),
                    "Accéder au ticket de l'utilisateur signalé")))
        .queue();

    embeds
        .successMessage(
            "Le ticket a été ouvert avec succès !\n:arrow_right: %s"
