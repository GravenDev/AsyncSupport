          ...
          case TicketOpeningReason.UserReport r -> {
            ...
            String reportedUser =
                user == null
                    ...
                    : "%s (`%s` ; `%s`)"
                        .formatted(user.getAsMention(), user.getAsTag(), user.getId());
    ...
    Executors.newSingleThreadExecutor()
        .execute(
            ...
            // spotless:on
              ...
              MessageFactory.create()
                  .addEmbeds(closedEmbed)
                  .send(from)
                  .queue(
                      ignored -> {},
                      error -> {
                        String errorMessage =
                            "Impossible d'informer l'utilisateur de la fermeture du ticket !";
